...
const url =
  process.env.NODE_ENV === "production"
    ? "http://localhost:3000"    //Change to production url in future
    : "http://localhost:3000";

const signupMail = async (to, name) => {
  try {
    const message = {
      from: "mr.kostidla@gmail.com",
      to,
      subject: "Welcome to Main Store",
      html: html(name),
    };
...
const forgotPasswordMail = async (token, email) => {
  try {
    const message = {
      ...
      <p><b>Note that this link will expire in the next one hour.</b></p>`,
    };
